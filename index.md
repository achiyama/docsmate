# Privacy policy for DocsMate developed by achiyama

## Chrome 拡張のプライバシーポリシー

本プライバシーポリシーは、achiyama（以下、「当開発者」）が開発した GoogleChrome の拡張機能である DocsMate(以下、「DocsMate」とします。)の利用において、利用者の個人情報もしくはそれに準ずる情報を取り扱う際に、当開発者が遵守する方針を示したものです。
...
拡張機能内において、現在閲覧しているページの URL を取得します。
取得された URL は、ブラウザ内の LocalStorage に保存されます。
